...
public partial class MainWindow : Window
{
    ...
    { _colorMode = ColorMode.Color;         ApplyColorModeState(); }

    private void ColorModeGray_Click(object sender, MouseButtonEventArgs e)
    { _colorMode = ColorMode.Grayscale;     ApplyColorModeState(); }

    private void ColorModeBW_Click(object sender, MouseButtonEventArgs e)
    { _colorMode = ColorMode.BlackAndWhite; ApplyColorModeState(); }
    ...
    { _saveNextToSource = false; ApplySegmentState(); }

    private void SegNextTo_Click(object sender, MouseButtonEventArgs e)
    { _saveNextToSource = true;  ApplySegmentState(); }
    ...
    private async void ConvertBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            string openTarget   = nextToSource ? Path.GetDirectoryName(files[0])! : outputFolder;
            string locationDesc = nextToSource ? "next to each source PDF" : outputFolder;

            var ask = MessageBox.Show(
                $"Converted {total} page(s) successfully.\n\nSaved to: {locationDesc}\n\nOpen output folder?",
                "Done", MessageBoxButton.YesNo, MessageBoxImage.Information);
            if (ask == MessageBoxResult.Yes)
                System.Diagnostics.Process.Start("explorer.exe", openTarget);
        }
        catch (Exception ex)
        {
